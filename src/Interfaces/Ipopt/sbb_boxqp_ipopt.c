...
Bool eval_f_sbb_boxqp(Index n, Number* x, Bool new_x,
...
Bool eval_grad_f_sbb_boxqp(Index n, Number* x, Bool new_x,
...
Bool eval_g_sbb_boxqp(Index n, Number* x, Bool new_x,
...
Bool eval_jac_g_sbb_boxqp(Index n, Number *x, Bool new_x,
		...
		Index *iRow, Index *jCol, Number *values, UserDataPtr user_data)
{
  ...
  if (values == NULL) {
	...
	for(j=i;j<cqp->ind_max_x;j++)
	  if (!Zero_BB(cqp->nb_var_y[ij2k(i,j,cqp->n)]))
	    {
	      if ((i==j) && ( (Negative_BB(cqp->nb_var_y[ij2k(j,j,cqp->n)]) && (j >= cqp->nb_int )) || (!Zero_BB(cqp->nb_var_y[ij2k(i,j,cqp->n)]) && (i<cqp->nb_int))))
		{
		  values[l+i] = - 1.0;
		  values[l+ind_y] = 1.0;
		  ind_y++;
...
Bool eval_h_sbb_boxqp(Index n, Number *x, Bool new_x, Number obj_factor,
...
/* Main Program */
int  eval_sbb_boxqp_ipopt(struct miqcp_bab bab,double ** sol_xy, double * sol_adm, int computeLocalSol) 
{
  ...
  nlp = CreateIpoptProblem(n, x_L, x_U, m, g_L, g_U, nele_jac, nele_hess,
			   index_style, &eval_f_sbb_boxqp, &eval_g_sbb_boxqp, &eval_grad_f_sbb_boxqp,
			   &eval_jac_g_sbb_boxqp, &eval_h_sbb_boxqp);
...
void sbb_boxqp_ipopt( struct miqcp_bab bab)
{
  ...
  int status = eval_sbb_boxqp_ipopt(bab,  &x_y,&sol_admissible,1);
  if(status ==1)
    {
  ...
  while(liste->size!=0)
      ...
      if (relative_gap  < EPS_BB )
	{
	 ...
	 if (cqp->nb_int > *i) 
	   {
	     new_bab_1 = update_variables_int_branch_1(*liste->first->bab, x_y,i,j);
		     
	     new_bab_2 = update_variables_int_branch_2(*liste->first->bab, x_y,i,j);
		     
	     suppress_first_liste(liste);
	     
	     if (test_bound_miqcp(new_bab_1,(cqp->nb_col-1)))
	       {
		 status = eval_sbb_boxqp_ipopt(new_bab_1, &x_y, &sol_admissible,1);
		 ...
		 if (ub_bb > sol_admissible)
		   {
		     status = eval_sbb_boxqp_ipopt(new_bab_2, &x_y, &sol_admissible,1);
		     if (status ==1)
		       {
			 absolute_gap = (sol_admissible -ub_bb);
			 relative_gap = absolute_gap/ub_bb;
	   ...
	   {
	     new_bab_1 = update_variables_cont_branch_1(*liste->first->bab, x_y,i,j);
	     new_bab_2 = update_variables_cont_branch_2(*liste->first->bab, x_y,i,j);
	     suppress_first_liste(liste);
	     ...
	     if (test_bound_miqcp(new_bab_1,(cqp->nb_col-1)))
	       {
		 if (x_inf_new_u)
		   status=eval_sbb_boxqp_ipopt(new_bab_1,  &x_y, &sol_admissible,0);
		 else
		   status =eval_sbb_boxqp_ipopt(new_bab_1, &x_y, &sol_admissible,1);
		 if (status ==1)
		   {
		 ...
		 if  (x_inf_new_u)
		   ...
		   status = eval_sbb_boxqp_ipopt(new_bab_2, &x_y,& sol_admissible,0);
		 if (status ==1)
		   {
		     if (ub_bb > sol_admissible)
		       {
