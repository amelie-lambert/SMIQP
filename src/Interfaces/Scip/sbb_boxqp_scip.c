...
static SCIP_RETCODE setupProblem_sbb_boxqp(SCIP* scip,struct miqcp_bab bab)
{
  ...
  SCIP_CALL( SCIPcreateProbBasic(scip, "sbb_boxqp") );
  ...
  for(i=0;i<cqp->ind_max_x;i++)
      ...
      if (!Zero_BB(cqp->nb_var_y[ij2k(i,j,cqp->n)]))
	{
	  if(i==j && i<cqp->nb_int)
	    ...
	    {
	      if(i==j &&(Negative_BB(cqp->nb_var_y[ij2k(i,i,cqp->n)])&& (i >= cqp->nb_int)))
		{
...
int eval_sbb_boxqp_scip(struct miqcp_bab bab,double ** sol_xy, double * sol_adm, int computeLocalSol)
{
   ...
   SCIP_CALL( setupProblem_sbb_boxqp(scip,bab));
 ...
 void sbb_boxqp_scip( struct miqcp_bab bab)
 {
  ...
  int status = eval_sbb_boxqp_scip(bab,  &x_y,&sol_admissible,1);
  if(status ==1)
    {
	 ...
	 if (cqp->nb_int > *i) 
	   {
	     ...
	     if (test_bound_miqcp(new_bab_1,cqp->nb_col -1))
	       {
		status = eval_sbb_boxqp_scip(new_bab_1, &x_y, &sol_admissible,1);
		if (status ==1)
		  {
	   ...
	   if (test_bound_miqcp(new_bab_2,cqp->nb_col -1))
	     {
		status = eval_sbb_boxqp_scip(new_bab_2, &x_y, &sol_admissible,1);
		if (status ==1)
		  {
		if (ub_bb > sol_admissible)
		  {
		...
		if (test_bound_miqcp(new_bab_1,cqp->nb_col -1))
		  {
		if (x_inf_new_u)
		  status=eval_sbb_boxqp_scip(new_bab_1,  &x_y, &sol_admissible,0);
		else
		  status =eval_sbb_boxqp_scip(new_bab_1, &x_y, &sol_admissible,1);
		if (status ==1)
		  {
		...
		if  (x_inf_new_u)
		  ...
		  status = eval_sbb_boxqp_scip(new_bab_2, &x_y,& sol_admissible,0);
		if (status ==1)
		  {
		if (ub_bb > sol_admissible)
		  {
